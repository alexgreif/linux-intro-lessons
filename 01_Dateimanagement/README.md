# Übung: Das Terminal als Dateimanager

In dieser Übung verwendest du das Terminal, um dich im Dateisystem zu orientieren und mit Dateien und Verzeichnissen (Ordnern) zu arbeiten.
Die Aufgaben werden schrittweise selbstständiger: Zuerst erkundest du das echte Dateisystem deiner Ubuntu-Server-VM, danach arbeitest du in einem eigenen Übungsverzeichnis, und zum Schluss stellst du eine durcheinandergeratene Lab-Struktur wieder her.

## Dateien

Für diese Übung benötigst du die folgenden Dateien:

- [Kurzreferenz](Kurzreferenz.md)
- [Wichtige Systemverzeichnisse](Wichtige_Systemverzeichnisse.md)
- [Aufgabe 1: Dateisystem erkunden](Aufgabe_1_Dateisystem_erkunden.md)
- [Aufgabe 2: Dateien und Verzeichnisse bearbeiten](Aufgabe_2_Dateien_und_Verzeichnisse.md)
- [Aufgabe 3: Dateisystem wiederherstellen](Aufgabe_3_Dateisystem_wiederherstellen.md)
- [Lab-Skript für Übung 1](lab_uebung_1.sh)

## Vorgehen

1. Lies zuerst die [Kurzreferenz](Kurzreferenz.md).
2. Bearbeite Aufgabe 1 und Aufgabe 2.
3. Lade die Kursunterlagen in der VM herunter. Folge dazu den [Anweisungen in der README im Hauptverzeichnis](../README.md#kursunterlagen-in-der-vm).
4. Wechsle in der VM in das Verzeichnis `~/kurs/linux-server-uebungen/01_Dateimanagement`.
5. Führe das Lab-Skript aus: `bash lab_uebung_1.sh`. Siehe dazu auch den [Abschnitt zu Lab-Skripten in der README im Hauptverzeichnis](../README.md#lab-skripte).
6. Bearbeite Aufgabe 3.

## Hinweise

- Prüfe regelmäßig, in welchem Verzeichnis du dich befindest.
- Kontrolliere Ausgaben, bevor du Dateien verschiebst oder löschst.
- Viele Aufgaben lassen sich auf unterschiedliche Weise lösen.

## Lernziele

- Pfade lesen und sicher zwischen Verzeichnissen wechseln
- Dateien und Verzeichnisse im Terminal finden und untersuchen
- Dateien und Verzeichnisse kopieren, verschieben, umbenennen und löschen
- Dotfiles, Symlinks, Wildcards und Shell-Expansion praktisch einordnen
- eine Zielstruktur selbstständig mit Terminalbefehlen wiederherstellen
